...
export class SaAthletesController {
  ...
  @Post()
  async saCreateAthlete(@Body() athleteDto: CreateAthleteInputModel) {
    const newAtleteId = await this.commandBus.execute(
      new SaCreateNewAtleteCommand(athleteDto),
    );
    const createdAthlete =
      await this.athletesRepository.getAthleteForSaById(newAtleteId);
    return createdAthlete;
  }

  @Get(':blogId')
  async getAthleteForSaById(@Param('blogId') blogId: string) {
